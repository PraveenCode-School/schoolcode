...
const StudentManagement = ({ config, prefillData, isPromotionView, defaultViewMode = 'active' }) => {
    ...
    // Gender Labels based on Institution Type
    const isCollege = config.institution_type === 'COLLEGE';
    const GENDER_LABELS = isCollege 
    ...
    const getGenderDisplay = (gender) => {
        ...
        if (g === 'male') return GENDER_LABELS.male;
        if (g === 'female') return GENDER_LABELS.female;
        return gender;
    };
                    ...
                    <tbody>
                        ${students.map(student => `
                            <tr>
                                ...
                                <td class="demo-cell">
                                    <div><span class="demo-label">Gender:</span> ${getGenderDisplay(student.gender)}</div>
                    ...
                    <button
                        onClick={() => {
                            import('xlsx').then(xlsx => {
                                ...
                                const data = [
                                    ['John', 'D', 'Doe', GENDER_LABELS.male, '2010-05-15', 'Class 10', 'A', 'Richard Doe', 'Jane Doe', '9876543210', 'student@example.com', '123 Main St']
                                ];
                                ...
                                <>
                                    {students.map(student => (
                                        <tr key={student.id} className="group hover:bg-slate-50/50 transition-colors">
                                            ...
                                            {!isPromotionView && (
                                                <td className="p-4">
                                                    <div className="text-slate-600 text-xs font-medium">{getGenderDisplay(student.gender)}</div>
